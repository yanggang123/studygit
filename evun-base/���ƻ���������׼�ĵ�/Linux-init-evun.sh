...
optimization_ssh()
{
    ...
    #cp /etc/nsswitch.conf{,.bak-$(date +%Y%m%d%H%M%S)}
    #[ -e /etc/nsswitch.conf ] && sed -i '/^hosts:/s/.*/#&/' /etc/nsswitch.conf
    #echo "hosts:      files"  >> /etc/nsswitch.conf
    #echo "End optimization the server ssh!!!"
}
...
sync_time()
{
    ...
    echo "*/5 * * * * root /usr/sbin/ntpdate 10.86.1.76 >> /home/timedate.log && hwclock -w && hwclock --show >> /home/timedate.log" >> /var/spool/cron/root
...
yum_conf ()
{
    ...
    wget -P /etc/yum.repos.d/ http://ops.evun.cn/evunsoft/CentOS6-evun.repo 
    yum clean all
    yum makecache
